...
fragment DeferredOut deferred_pbr_fs_main(FSIn input [[stage_in]],
	...
	float3 worldPos = WorldPosFormDepth(input.uv, scene.ip, depth.sample(smp, input.uv).x);
	...
	for (int i = 0; i < MAX_LIGHTS; ++i) {
		...
		float att = 1.0f / dot(light.att, float3(1.0f, l_distance, l_distance * l_distance));
